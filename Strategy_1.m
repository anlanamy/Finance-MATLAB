...
function Signal = Strategy_1(Asset,HisDB,windcode)
Signal = [];
for i = 1:length(windcode)
    ...
    if length(db.Close)>=21 && db.Close(end) <= mean(db.Close(end-20:end))
...
%        Signal{i}.Volume=0;
        if length(db.Close)~=1
            Signal{i}.Volume = Asset.Cash(length(db.Close)-1)/(6*db.Close(end));
        else
            Signal{i}.Volume = Asset.InitCash/(6*db.Close(end));
        end
%        for j = 1:length(Asset.CurrentPosition)
%             if isequal(Asset.CurrentStock{j},namei)
%                 %Signal{i}.Volume = -Asset.CurrentPosition(j);
